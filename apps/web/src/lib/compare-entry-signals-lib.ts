/**
 * Pure compare decision signal helpers.
 *
 * Builds trust/review/provenance signal values for compare drawers and tables.
 * Nothing here touches the network — given the same entry metadata the output
 * is deterministic.
 *
 * The public surface (`compare-entry-signals.ts` / `@/lib/compare-entry-signals`)
