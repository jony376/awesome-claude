/**
 * Pure newsletter email renderers.
 *
 * This module holds the deterministic HTML/text template layer for HeyClaude
 * transactional and broadcast emails. Nothing here touches the network — given
 * the same inputs the output is byte-stable.
 *
 * The public surface (`newsletter-emails.ts` / `@/lib/newsletter-emails`)
...
export const BODY = "'DM Sans',-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif";
...
export function trimUrl(siteUrl: string): string {
...
export function emailShell(opts: { preheader: string; inner: string }): string {
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
export function standingCtasHtml(siteUrl: string): string {
...
export function standingCtasText(siteUrl: string): string {
  const base = trimUrl(siteUrl);
  return [
    "—",
...
export function buildNewsletterConfirmEmail(opts: { confirmUrl: string; siteUrl: string }): {
  ...
  return {
    subject,
    html: emailShell({
      preheader: "Confirm your subscription to the HeyClaude weekly brief.",
      inner,
    }),
    text,
  };
...
export function buildWelcomeEmail(opts: { siteUrl: string }): {
  ...
  return {
    subject,
    html: emailShell({ preheader: "Welcome — your weekly Claude brief lands Sundays.", inner }),
    text,
  };
}
...
export function buildListingLeadAckEmail(opts: {
  ...
  return {
    subject,
    html: emailShell({
      preheader: "We received your inquiry — a maintainer will be in touch.",
      inner,
    }),
    text,
  };
