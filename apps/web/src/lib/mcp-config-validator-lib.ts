...
export function normalizeServerName(value: string) {
  return value.trim();
}
...
export function redactArgArray(values: unknown[]): SanitizedValue {
  let redactedCount = 0;
...
export function sanitizeConfigValue(key: string, value: unknown): SanitizedValue {
...
export function firstPackageOperand(args: string[]) {
...
export function packageRunnerName(command: string, args: string[]) {
...
export function packageFromRunner(command: string, args: string[]) {
  const lower = commandName(command);
...
export function buildReportText(result: Omit<McpConfigValidation, "reportText">) {
  const lines = [
