/**
 * Pure weekly-brief email renderers.
 *
 * This module holds the deterministic HTML/text newsletter layer used for
 * maintainer review previews and audience sends. Nothing here touches the
 * network — given the same brief payload the output is byte-stable.
 *
 * The public surface (`brief-email.ts` / `@/lib/brief-email`) re-exports
...
export const FEATURED_PER_SECTION = 4;
...
export function itemBadges(item: BriefItem): string[] {
  const badges: string[] = [];
...
export function cardHtml(item: BriefItem, siteUrl: string): string {
...
export function overflowRowHtml(item: BriefItem, siteUrl: string): string {
  const href = escapeHtml(absolute(String(item.url ?? ""), siteUrl));
  const title = escapeHtml(String(item.title ?? ""));
  const cat = escapeHtml(categoryLabel(item.category));
  const catTag = cat
