...
usageSnippet: "⚠️ API schema drift in openapi.json: removed path(s): /v1/users/{id}"
copySnippet: |-
  {
    "hooks": {
      "PostToolUse": [
...
scriptBody: |-
  ...
  if [ -n "$removed_paths" ]; then
    echo "⚠️ API schema drift in $FILE: removed path(s):" >&2
    printf '   - %s\n' $removed_paths >&2
    drift=1
  fi
  ...
  if [ -n "$removed_required" ]; then
    echo "⚠️ API schema drift in $FILE: removed required field(s):" >&2
    printf '   - %s\n' $removed_required >&2
    drift=1
  fi
...
prerequisites:
  ...
  - git, jq, and bash on PATH (the hook fails open and stays silent if git or jq is missing).
  - The schema file must be tracked in git so a committed baseline (HEAD) exists to diff against.
safetyNotes:
  - Runs after every Write, Edit, and MultiEdit and only inspects JSON files that declare openapi, swagger, $schema, or paths.
  - Read-only: it runs git show to read the committed version and never edits, stages, or commits anything.
  - Advisory by design — it always exits 0 and never blocks a write, even when drift is detected.
...
privacyNotes:
  ...
  - Prints removed path names and required-field names to local hook stderr; it makes no network calls and writes no logs.
